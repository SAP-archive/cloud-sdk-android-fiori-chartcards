...
import com.example.fiorichartcardsapp.databinding.FragmentScrollableCardsBinding
...
class ScrollableCardsFragment : Fragment() {

    private lateinit var binding: FragmentScrollableCardsBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentScrollableCardsBinding.inflate(inflater, container, false)
        binding.lifecycleOwner = viewLifecycleOwner
        binding.chartCardsViewModel = viewModel
        mChartCardView = binding.root.findViewById(R.id.chartCardViewScrollable)

        viewModel.covidStatesChartCards.observe(viewLifecycleOwner,
            Observer<MutableList<ChartCardDataModel>> {
                if (it.isNotEmpty())
                    setChartCardView(it)
            })

        viewModel.xLabels4StatesData.observe(viewLifecycleOwner,
    ...
    private fun setChartCardView(statesChartCards: MutableList<ChartCardDataModel>) {
        val layoutType = binding.chartCardViewScrollable.layoutType
        (binding.chartCardViewScrollable).setViewLayoutManager()
        val chartCardViewAdapter = this.activity?.let {
            ChartCardViewAdapter(it, statesChartCards, layoutType)
        }
        (binding.chartCardViewScrollable).setViewAdapter(chartCardViewAdapter)

        mChartCardView?.setItemClickListener(object : FioriItemClickListener {
            override fun onClick(view: View, position: Int) {
                val card = statesChartCards[position]
